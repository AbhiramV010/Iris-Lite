...
static inline uint64_t wrap(uint64_t v, uint64_t mod)
{
    return mod ? (v % mod) : v;
}
...
bool CompressionEngine::initialize(const Config& cfg)
{
    ...
    encoder = std::make_unique<H264Encoder>(
        ...
        cfg.encodeHeight,
        cfg.fps,
        true // hardware preferred
    );

    stop = false;
    worker = std::thread(&CompressionEngine::workerLoop, this);

    logInfo("CompressionEngine initialized (FINAL)");
    return true;
}
...
void CompressionEngine::enqueueEvent(const EventWindow& event)
{
    ...
    {
        queue.pop();
        logWarn("Queue overflow: dropping oldest event");
    }
